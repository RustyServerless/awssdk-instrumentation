...
///
/// `OtelInstrumentor` creates an OTel span named `Invocation` directly via the
/// global OTel tracer for each Lambda invocation. The span has `internal` kind
/// by default, or `server` kind when the `xray-no-lambda-node-nesting` feature
/// is enabled, and carries the standard FaaS attributes.
///
...
impl Instrumentor for OtelInstrumentor {
    ...
    fn instrument<F: Future>(inner: F, context: super::InvocationContext) -> Self::IFut<F> {
        ...
        let span_builder = SpanBuilder::from_name("Invocation")
            .with_kind(kind)
            .with_attributes([
                ...
                KeyValue::new(semco::SERVICE_NAME, context.function_name),
            ]);
        ...
        }) = context.xray_trace_header
        {
            ...
            // Add the xray_trace_id attribute
            let mut span_builder = span_builder;
            span_builder
                .attributes
                .as_mut()
                .expect("attributes set above")
                .push(KeyValue::new("xray_trace_id", trace_id.to_string()));
            otel_context.with_span(tracer.build_with_context(span_builder, &otel_context))
        } else {
